<template>
  <div class="fileComp">
    ...
    >
      <el-table-column label="文件名" min-width="200">
        <template slot-scope="scope">
          <span>
            <i :class="classifyIcon(scope.row)" style="font-size: 30px;margin-right: 10px;cursor: pointer;"></i>
            ...
            <span v-if="scope.row.newFolder">
              <el-input @keyup.enter.native="newFoler"
                        ...
                        style="width: 70%;display: inline-block;position: relative;top:0;">
              </el-input>
            </span>
          </span>
        </template>
      </el-table-column>
      <el-table-column width="200px" label="类型">
        <template slot-scope="scope">
          <span v-if="scope.row.folder == false">{{scope.row.fileEntity.type}}</span>
          <span v-if="scope.row.folder == true">--</span>
      ...
      <el-table-column width="200px" label="大小">
        <template slot-scope="scope">
      ...
      <!--<uploader :options="options"
        ...
        <uploader-unsupport></uploader-unsupport>
          <uploader-drop>
            ...
            &lt;!&ndash;<uploader-btn :directory="true">选择文件夹</uploader-btn>&ndash;&gt;
          </uploader-drop>
        <uploader-list ref="uploaderList"></uploader-list>
      </uploader>-->
      <fileUp ref="fileup" :parentNodeId="parentNodeId" :componentId="selectCompId"></fileUp>
    ...
    <el-dialog
      ...
      <folderUp :parentNodeId="parentNodeId" :componentId="selectCompId" ref="folderUp"></folderUp>
      <span slot="footer" class="dialog-footer">
...
<script>
  ...
  import maniFile from './maniFile'
  import qs from 'qs'
  import fileUp from './file-uploader'
  import folderUp from './folder-uploader'
  export default {
    name: 'comFileManage',
    props: {
      selectCompId: {
    ...
    methods: {
      ...
      handleuploadFile() {
        ...
        this.$nextTick(() => {
          // this.$refs.fileUp.uploader.uploader.cancel() //清空文件上传列表
        })
      },
      handleUploadFolder() {
        this.uploadFolderDialog = true
        this.$nextTick(() => {
          // this.$refs.folderUp.$refs.uploader.cancel() //清空文件上传列表
        })
      },
      uploadFile() {
        this.listLoading = true
